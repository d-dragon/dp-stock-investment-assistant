...
3. Existing target `week-WW.json` (upsert; never replace sibling days)
4. Golden file `2026-09/week-38.json` if unsure about a field
...
| Tin thế giới | `report.globalNews[]` → `date`, `title`, `summary`, `impact.text`, `image.url`, `sourceId` |
| Vĩ mô VN | `report.vietnamMacro[]` → `nameVi`, `value`, `unit` + `report.vietnamNews[]` |
