(function () {
  ...
  function boot() {
    ...
    const reportP = window.__REPORT__ ? Promise.resolve(window.__REPORT__) : fetch(path).then(function (r) {
    ...
    const sourcesP = window.__SOURCES__
      ...
      : srcPath
        ? fetch(srcPath).then(function (r) { return r.ok ? r.json() : { sources: [] }; }).catch(function () { return { sources: [] }; })
        : Promise.resolve({ sources: [] });
    Promise.all([reportP, sourcesP]).then(function (pair) {
      const rep = pair[0];
      const src = pair[1];
      ...
      REPORT = rep;
      (src.sources || []).forEach(function (s) { SOURCES[s.id] = s; });
      renderDash();
    }).catch(function (e) {
  ...
  function renderHub(cat) {
    const rows = (cat.reports || []).map(function (r) {
      const href = "./" + (r.path || "");
      return '<li class="hub-row"><a href="' + href + '">' + r.date + ' \u2014 ' + (r.title || '') + '</a> <span class="badge ' + (r.status || '') + '">' + (r.status || '') + '</span></li>';
    }).join("");
    return '<header class="hdr"><h1>DP Stock \u2014 Ban tin thi truong</h1></header><div class="pb" style="padding:16px"><ul class="news">' + rows + '</ul></div>';
  }

  function sessionLabel(s) {
    if (s === "closed") return "da dong cua";
    if (s === "intraday") return "phien dang chay";
    return "ngoai gio";
  }
  ...
  function renderDash() {
    ...
    document.getElementById("app").innerHTML =
      '<header class="hdr"><h1>Ban tin thi truong</h1><div class="meta"><span>' + (r.timezone || '') +
      '</span><span>' + (r.generatedAt || '') + '</span><span class="badge ' + sess + '">' + sessionLabel(sess) +
      '</span></div><button id="themeBtn" type="button">Giao dien</button></header>' +
  ...
  function renderNews(key, listId, pagerId, which) {
    ...
    document.getElementById(pagerId).innerHTML = '<button type="button" data-d="-1">Truoc</button><span>Trang ' + (page + 1) + '/' + pages + ' (' + items.length + ' tin)</span><button type="button" data-d="1">Sau</button>';
  ...
  function renderChart(tab) {
    ...
    if (tab === "vnindex") {
      ...
      pane.innerHTML = '<div class="chart-wrap"><div class="chart-main"><div class="stats">' +
        ...
        '<div class="stat"><b>GTGD (ty)</b>' + num(eq.liquidity && eq.liquidity.hose && eq.liquidity.hose.valueVndBn) + '</div>' +
        '<div class="stat"><b>NN rong (ty)</b><span class="' + cls(eq.foreign && eq.foreign.netValueVndBn) + '">' + num(eq.foreign && eq.foreign.netValueVndBn) + '</span></div>' +
        '<div class="stat"><b>Tu doanh</b>' + ((eq.proprietary && eq.proprietary.netValueVndBn != null) ? num(eq.proprietary.netValueVndBn) : ((eq.proprietary && eq.proprietary.note) || '\u2014')) + '</div>' +
        '<div class="stat"><b>Ca nhan</b>' + ((eq.retail && eq.retail.netValueVndBn != null) ? num(eq.retail.netValueVndBn) : ((eq.retail && eq.retail.note) || '\u2014')) + '</div>' +
  ...
  function drawLine(canvasId, ser, label) {
    ...
    const extra = ser && ser.interpolation === "anchored" ? " (noi suy)" : "";
  ...
  function drawBars(canvasId) {
    ...
    charts[canvasId] = new Chart(ctx, {
      ...
      data: { labels: v.labels, datasets: [
        { label: "GTGD (nghin ty)", data: v.valueVndTn, backgroundColor: "#3b82f6", yAxisID: "y" },
        { label: "NN rong (ty)", data: (f && f.values) || [], type: "line", borderColor: "#f59e0b", yAxisID: "y1", pointRadius: 3 }
      ] },
  ...
  function renderFoot() {
    const links = Object.keys(SOURCES).map(function (id) {
      const s = SOURCES[id];
      return '<a href="' + s.url + '" target="_blank" rel="noopener noreferrer">' + s.name + '</a>';
    }).join(' \u00b7 ');
