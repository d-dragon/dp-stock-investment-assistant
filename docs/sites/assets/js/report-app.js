(function () {
  ...
  function boot() {
    ...
    Promise.all([
      ...
      window.__SOURCES__ ? Promise.resolve(window.__SOURCES__) : (srcPath ? fetch(srcPath).then((r) => r.json()).catch(function () { return { sources: [] }; }) : Promise.resolve({ sources: [] }))
    ]).then(function (pair) {
      var rep = pair[0], src = pair[1];
      ...
      document.getElementById("app").innerHTML = '<div class="err">Không tải được dữ liệu (' + e.message + ').</div>';
    });
  }
  function renderHub(cat) {
    const rows = (cat.reports || []).map(function (r) {
      return '<li style="padding:8px 0;border-bottom:1px solid var(--border)"><a href="./" + r.path + '">' + r.date + ' — ' + r.title + '</a> <span class="badge ' + r.status + '">' + r.status + '</span></li>';
    }).join("");
