...
ADR-Frontend-001 establishes that the frontend should evolve as a modular application rather than adopt micro-frontends now. That architectural direction is necessary, but not sufficient. The repository also needs an explicit decision on the modernization foundation that will implement that architecture.
