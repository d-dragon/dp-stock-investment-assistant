...
**Generated**: 2026-01-28 | **Estimated**: 25-31 Story Points
...
| 2: Foundational (BLOCKING) | 8 | 6-7 SP | Schema, repository, config validation |
...
| 9: STM Runtime Integration (CRITICAL) | 5 | 5-6 SP | Agent migration, checkpointer wiring |
| **Total** | **40** | **25-31 SP** | |
...
## Phase 9: STM Runtime Integration & Agent Migration (CRITICAL) — 5-6 SP

> **Goal**: Make STM actually functional at runtime. Phases 1-8 built all components correctly but left **three critical wiring gaps** that prevent STM from working when the app runs. This phase fixes the integration seams so `/api/chat` with `session_id` produces persistent multi-turn conversations in MongoDB.
>
> **Problem Summary**:
> - Gap 0: `create_react_agent` deprecated — migrate to `create_agent`
...
- [X] T038 [P] Migrate `create_react_agent` → `create_agent` in `src/core/stock_assistant_agent.py`
  - Change import: `from langgraph.prebuilt import create_react_agent` → `from langchain.agents import create_agent`
  - In `_build_agent_executor()`: rename `prompt=` → `system_prompt=`
  - Add `name="stock_assistant"` parameter for subgraph compatibility
  - Verify return type is still `CompiledStateGraph`
  - Update module docstring with migration note
  - **Verification**: `pytest tests/test_agent.py tests/test_agent_memory.py -v`

### Config Fix

- [X] T039 [P] Fix MongoDB config key mismatch in `src/core/langgraph_bootstrap.py`
  - Fix config path: `config.get("mongodb", {})` → `config.get("database", {}).get("mongodb", {})`
  - Fix key name: `.get("uri", "")` → `.get("connection_string", "")`
  - Fix DB name key: `.get("database", ...)` → `.get("database_name", ...)`
  - Add debug logging for resolved connection params (mask password)
  - **Verification**: Start with MongoDB running; check logs for "MongoDBSaver checkpointer initialized"

### Checkpointer Wiring

- [X] T040 Wire checkpointer into agent construction in `src/web/api_server.py` and `src/main.py`
  - Import `create_checkpointer` from `core.langgraph_bootstrap` in `api_server.py`
  - In `APIServer.__init__()`, call `checkpointer = create_checkpointer(self.config)`
  - Pass `checkpointer=checkpointer` to `StockAssistantAgent(...)` constructor
  - Log checkpointer status at startup: `Agent memory: enabled/disabled`
  - Apply same wiring in `src/main.py` for `cli` and `both` modes
  - **Depends on**: T038, T039
  - **Verification**: Start server → `GET /api/health` → check startup logs for "Agent memory: enabled"

### Runtime Verification

- [X] T041 Create end-to-end runtime verification test in `tests/integration/test_stm_runtime_wiring.py`
  - Test: Start app with MongoDB running and `langchain.memory.enabled: true`
  - Test: `POST /api/chat` with `message + session_id` → verify 200 response
  - Test: `POST /api/chat` with follow-up + same `session_id` → verify context recall
  - Test: Query `agent_checkpoints` collection → verify data persisted for `thread_id`
  - Test: Query `conversations` collection → verify metadata for `session_id`
  - Test: Simulate restart (reinitialize agent) → verify persistence across restart
  - **Depends on**: T040

### Test Migration

- [X] T042 Update existing test mocks for `create_agent` migration in `tests/test_agent.py`, `tests/test_agent_memory.py`, `tests/test_langsmith_integration.py`
  - Update mock patches from `langgraph.prebuilt.create_react_agent` → `langchain.agents.create_agent`
  - Verify mock signatures match new API (`system_prompt` instead of `prompt`)
  - Run full test suite: `pytest -v` — all tests must pass
  - **Depends on**: T040

> **Phase 9 Completed** — All 5 tasks (T038-T042) implemented and verified.
>
> **Summary**:
> - **T038**: Migrated `create_react_agent` → `create_agent` (import + params + docstring)
> - **T039**: Fixed 3 config keys in `create_checkpointer()` + added `_mask_connection_string()` helper
> - **T040**: Wired checkpointer into `APIServer.__init__()` and `main.py` (cli/both modes)
> - **T041**: Created 19 runtime verification tests (7 classes) — all passing
> - **T042**: Updated old `mongodb.uri` refs in test fixtures + skill docs; no `create_react_agent` refs in tests
>
> **Test Results**: 599 passed, 4 failed (pre-existing), 4 skipped
> **Package Upgrade**: langchain 0.3.27 → 1.2.9, langgraph 1.0.8 installed, langgraph-checkpoint 4.0.0
...
| T038 | T001-T037 | Agent API migration after all components built |
| T039 | T001-T037 | Config fix after all components built |
| T040 | T038, T039 | Checkpointer wiring needs both migration and config fix |
| T041 | T040 | E2E verification needs full wiring complete |
| T042 | T040 | Test migration needs new API integrated |
...
**Phase 9 (STM Runtime Integration)**:
- T038 + T039 can run in parallel (different files, no dependencies)
- T041 + T042 can run in parallel after T040 completes
