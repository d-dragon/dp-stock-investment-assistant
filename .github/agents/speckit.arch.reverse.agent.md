...
1. **Setup**: Run `.specify/extensions/arch/scripts/powershell/setup-arch.ps1 -Json` from repo root and parse JSON for `ARCH_FILE`, `ARCH_DIR`, `REPO_FACTS_FILE`, `SCENARIO_VIEW`, `LOGICAL_VIEW`, `PROCESS_VIEW`, `DEVELOPMENT_VIEW`, and `PHYSICAL_VIEW`.
