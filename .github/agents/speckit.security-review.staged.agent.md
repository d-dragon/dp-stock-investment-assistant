...
2. **Identify Changed Files**:
   - You **MUST** execute the `.specify/scripts/powershell/detect-changed-files.ps1` with `--json` to detect changed files in the working directory (`Mode B`).
...
1. **Identify Scope**: Run `.specify/scripts/powershell/detect-changed-files.ps1` to identify the changed files.
...
```text
| .specify/extensions/security-review/docs/security-reviews/2026-05-07-staged.md | staged | 2026-05-07 | MODERATE | C:0 H:1 M:2 L:1 | A02,A09 |
```
