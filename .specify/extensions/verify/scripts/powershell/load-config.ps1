...
#
# Reads report.max_findings from the YAML config file, normalizes YAML
# null sentinels, applies an optional environment variable override
# (SPECKIT_VERIFY_MAX_FINDINGS), and validates that a value is present
# before exporting it.
#
...
if ($usingDefaults) {
    Write-Host "No config file found; using defaults from extension.yml"
}
