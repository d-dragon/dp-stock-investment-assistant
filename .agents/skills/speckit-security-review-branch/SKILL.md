...
2. **Identify Target & Base**:
   ...
   - Otherwise, you **MUST** execute the `.specify/scripts/powershell/detect-changed-files.ps1` with `--json` to detect changed files between the feature branch and the default branch (`Mode A`).
...
1. **Identify Scope**: Run `.specify/scripts/powershell/detect-changed-files.ps1` or use user-provided branch names to identify the changed files. If using branches, determine `<base>` and `<target>`.
